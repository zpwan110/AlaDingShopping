...
import com.alading.library.exception.FileAlreadyExistException;
...
public class FileHttpResponseHandler extends AsyncHttpResponseHandler
{
	...
	{
		onSuccess(urlPath,new String(binaryData));
	}
	...
	protected void sendSuccessMessage(int statusCode, byte[] responseBody,String requset)
	{
		...
		{requset, statusCode, responseBody }));
	}
	...
	protected void handleSuccessMessage(int statusCode, byte[] responseBody,String requset)
	{
		onSuccess(requset,statusCode, responseBody);
	}

	protected void handleFailureMessage(Throwable e, byte[] responseBody,String requset)
	{
		onFailure(requset, e, responseBody);
	}
	...
	protected void handleMessage(Message msg)
	{
		...
		case SUCCESS_MESSAGE:
			...
			handleSuccessMessage(((Integer) response[1]).intValue(),
					(byte[]) response[2],(String) response[0]);
			break;
		default:
	...
	protected void sendResponseMessage(HttpResponse response,String requset)
	{
		...
		{
			// TODO Auto-generated catch block
			sendFailureMessage(e, responseBody,requset);
			error = e;
		...
		if (result == -1 || interrupt || error != null)
		{
			if (error != null)
			{
				...
				{
					sendSuccessMessage(statusCode, "下载成功！".getBytes(),requset);

				} else
				{
					sendFailureMessage(error, responseBody,requset);
				}
			}
			return;
		}
